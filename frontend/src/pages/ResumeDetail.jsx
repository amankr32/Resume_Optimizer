...
import { Sparkles, ArrowLeft, Loader2, FileText, Download, Briefcase, ChevronDown } from "lucide-react";
  ...
  async function runApplyRewrites(rewriteIds) {
    ...
    try {
      ...
      if (res?.version?._id) {
        ...
        setTab("score");
        // Auto-analyze the new version with the same target role/job description
        // so the user immediately sees whether their rewrites moved the score.
        try {
          await analyze.mutateAsync({
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <button
          ...
          className="mt-4 flex items-center gap-2 text-xs font-semibold text-[var(--accent-strong)] hover:opacity-80 transition-opacity"
        >
          <Briefcase size={13} />
          {showJobDescription ? "Hide job description" : "Match against a job description"}
          {jobDescription.trim() && !showJobDescription && (
            <Badge tone="success" className="ml-1">JD added</Badge>
          )}
          <ChevronDown
            size={13}
            className={`transition-transform ${showJobDescription ? "rotate-180" : ""}`}
          />
        </button>
        ...
        {showJobDescription && (
          <div className="mt-3 space-y-2">
            <Textarea
              placeholder="Paste the full job description here. The AI will score this resume specifically against these requirements and keywords."
              rows={7}
            ...
            <div className="flex items-center justify-between text-[11px] text-[var(--ink-muted)]">
              <span>
                Optional — when provided, keyword matching and the ATS score are computed
                against this specific job description instead of a general role.
              </span>
                ...
                {analysis.jobDescription && (
                  <div className="mb-3">
                    <Badge tone="success" className="gap-1.5">
                      <Briefcase size={11} /> Matched against your job description
                    </Badge>
                  </div>
                )}
