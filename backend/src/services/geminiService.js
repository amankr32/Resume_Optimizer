...
function buildPrompt({ rawText, targetRole, jobDescription }) {
  ...
  return [
    ...
    hasJD
      ? "A specific job description has been provided below. Treat it as the primary source of truth: extract its required skills, qualifications, and keywords, and score/assess the resume specifically against THIS job description rather than a generic role."
      : targetRole
      ? `Target role: ${targetRole}.`
      : "No specific target role or job description was provided - assess for the role the candidate appears to be aiming for.",

    "Score the resume from 0-100 based on ATS readiness (keyword match, parseable formatting, quantified impact, clarity).",

    hasJD
      ? "The 'keywords' portion of the score breakdown must reflect how well the resume covers the specific requirements and keywords found in the job description below."
      : null,
    ...
    hasJD
      ? "For keywordsPresent and keywordsMissing, use the specific skills/requirements/technologies named in the job description below - keywordsPresent are ones the resume already demonstrates, keywordsMissing are JD requirements the resume does not clearly show."
      : "Identify keywords clearly present and notable keywords missing for the apparent target role.",

    hasJD
      ? "In the summary, explicitly call out how well this resume matches the job description and the single biggest gap to close."
      : null,
